...
class PlayState extends MusicBeatState {
	...
	public var scriptPack:ScriptPack = new ScriptPack("PlayState");
	...
	public function startCountdown() {
		...
		if (ret != LuaUtils.Function_Stop) {
			...
			startTimer = new FlxTimer().start(Conductor.crochet / 1000 / playbackRate, function(tmr:FlxTimer) {
				...
				callOnNxScript('onCountdownTick', [tick, swagCounter]);
	...
	override public function update(elapsed:Float) {
		...
		if (unspawnNotes[0] != null) {
			...
			while (unspawnNotes.length > 0 && unspawnNotes[0].strumTime - Conductor.songPosition < time) {
				...
				callOnNxScript('onSpawnNote', [dunceNote]);
	...
	public function endSong() {
		...
		if (ret != LuaUtils.Function_Stop && !transitioning) {
			...
			if (!cpuControlled && !chartingMode && !practiceMode) { // anti-cheat
	...
	function noteMiss(daNote:Note):Void { // You didn't hit the key and let it go offscreen, also used by Hurt Notes
		...
		if (result != LuaUtils.Function_Stop && result != LuaUtils.Function_StopNxScript && result != LuaUtils.Function_StopAll)
			result = callOnNxScript('noteMiss', [daNote]);
	}
	...
	function opponentNoteHit(note:Note):Void {
		...
		if (result != LuaUtils.Function_Stop && result != LuaUtils.Function_StopNxScript && result != LuaUtils.Function_StopAll)
			result = callOnNxScript('opponentNoteHitPre', [note]);
		...
		if (result != LuaUtils.Function_Stop && result != LuaUtils.Function_StopNxScript && result != LuaUtils.Function_StopAll)
			result = callOnNxScript('opponentNoteHit', [note]);
	...
	public function goodNoteHit(note:Note):Void {
		...
		if (result != LuaUtils.Function_Stop && result != LuaUtils.Function_StopNxScript && result != LuaUtils.Function_StopAll)
			result = callOnNxScript('goodNoteHitPre', [note]);
		...
		if (result != LuaUtils.Function_Stop && result != LuaUtils.Function_StopPython && result != LuaUtils.Function_StopAll)
			result = callOnPython('goodNoteHit', [note]);
		if (result != LuaUtils.Function_Stop && result != LuaUtils.Function_StopNxScript && result != LuaUtils.Function_StopAll)
			result = callOnNxScript('goodNoteHit', [note]);
		if (!note.isSustainNote)
			invalidateNote(note);
	}
	...
	#if NXSCRIPT_ALLOWED
	public function startNxScriptNamed(scriptFile:String) {
		#if MODS_ALLOWED
		var scriptToLoad:String = Paths.modFolders(scriptFile);
		if (!FileSystem.exists(scriptToLoad))
			scriptToLoad = Paths.getSharedPath(scriptFile);
		...
		if (FileSystem.exists(scriptToLoad)) {
			initNxScript(scriptToLoad);
			return true;
		}
		return false;
	}

	public function initNxScript(file:String) {
		try {
			var newScript:NxScript = new NxScript(file);
			CoolLog.info('initialized NxScript interp successfully: $file');
			scriptPack.add(newScript);
			newScript.execute();
		} catch (e:Dynamic) {
	...
	public function setOnScripts(variable:String, arg:Dynamic, exclusions:Array<String> = null) {
		...
		for (script in scriptPack.scripts) {
			...
			if (scriptOrigin == null && Std.isOfType(script, HScript)) {
				var hscriptScript:HScript = cast script;
			...
			#if NXSCRIPT_ALLOWED
			if (scriptOrigin == null && Std.isOfType(script, NxScript)) {
				var nxScript:NxScript = cast script;
				scriptOrigin = nxScript.origin;
			}
			#end
