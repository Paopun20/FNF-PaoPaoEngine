package funkin.backend.mobile.states;

import flixel.effects.FlxFlicker;
import mikolka.vslice.freeplay.FreeplayState;
import mikolka.vslice.ui.title.TitleState;
import mikolka.funkin.custom.funkin.backend.mobile.MobileScaleMode;
import funkin.backend.mobile.objects.GridButtons;
import flixel.FlxBasic;
import mikolka.compatibility.VsliceOptions;
import funkin.backend.mobile.objects.grid.*;
#if !LEGACY_PSYCH
#if MODS_ALLOWED
...
class MobileMenuState extends FlxBasic {
	...
	override function update(elapsed:Float) {
		if (!selectedSomethin) {
			...
			if (#if TOUCH_CONTROLS_ALLOWED host.touchPad.buttonC.justPressed
				|| #end#if LEGACY_PSYCH FlxG.keys.anyJustPressed(ClientPrefs.keyBinds.get('debug_1')
