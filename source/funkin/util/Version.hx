...
class Version
{
	private static final PRE_ORDER:Map<String, Int> = ["dev" => 0, "alpha" => 1, "beta" => 2, "rc" => 3];
	...
	private static function prereleaseIsOlder(a:Null<String>, b:Null<String>):Bool
	{
		...
		if (a == null && b != null)
			return false; // current is release, target is pre → not outdated
		if (a != null && b == null)
			return true; // current is pre, target is release → outdated
		if (a == null && b == null)
			return false; // both are releases → not outdated
		...
		if (pa == null)
			return false; // unknown current is treated as newer
